...
path2pfamseq = '/Users/kreynolds/Documents/Packages/pfamseq.txt'
...
def clean_al(alg, code='AGUC', gap='-'):
...
def MSAsearch(hd, algn, seq, species = None, path2_algprog=path2needle):
    ...
    if species is not None:
        ...
        for h in hd:
            hs = h.split('|')
            hd_species.append(hs[2])
        key_list = list()
        for (i,h) in enumerate(hd_species):
...
def lett2num(msa_lett, code='AGUC'):
...
def seqWeights(alg, max_seqid=.8, gaps=1):
    ...
    **Keyword Arguments:**
    ...
    if gaps == 1: codeaa += '-'
    if gaps == 1: codenuc += '-'
    msa_num = lett2num(alg, code=codenuc)
    X2d = alg2bin(msa_num, N_aa=len(codenuc) )
...
def freq(alg, seqw=1, Naa=4, lbda=0, freq0=np.ones(4)/5):
    '''
    ...
    **Returns:**
    ...
    #freq0_reg = freq1_reg.reshape(Npos, 20).mean(axis=0)
    freq0_reg = freq1_reg.reshape(Npos, 4).mean(axis=0)
    return freq1_reg, freq2_reg, freq0_reg

    

def dcfreq(alg, seqw=1, Naa=4, lbda=0, freq0=np.ones(4)/5):
    '''
    Compute amino acid frequencies for a given alignment.
    ...
    **Returns:**
    ...
    al2d = alg2bin(alg, Naa)
    freq1 = seqw.dot(np.array(al2d.todense()))[0]
    freq2 = np.array(al2d.T.dot(scipy.sparse.diags(seqw[0], 0)).dot(al2d).todense())
    # Background:
    block = np.outer(freq0,freq0)
    freq2_bkg = np.zeros((Npos*Naa, Npos*Naa))
    for i in range(Npos): freq2_bkg[Naa*i:Naa*(i+1),Naa*i:Naa*(i+1)] = block
    ...
    # Regularizations:
    #freq1_reg = (1-lbda)*freq1 + lbda*np.tile(freq0,Npos)
    freq1_reg = (1.0/(2*Meff))*(freq1 + Meff*np.tile(freq0,Npos))
    #freq2_reg = (1-lbda)*freq2 + lbda*freq2_bkg
    freq2_reg = (1.0/(2*Meff))*(freq2 + Meff*freq2_bkg)
    #freq0_reg = freq1_reg.reshape(Npos, 20).mean(axis=0)
    freq0_reg = freq1_reg.reshape(Npos, 4).mean(axis=0)
    return freq1_reg, freq2_reg, freq0_reg
...
def posWeights(alg, seqw=1, lbda=0, freq0 = np.array([.2706, .306, .207, .217])): #AGUC
    ...
    **Returns:**
    ...
    '''
    N_seq, N_pos = alg.shape; N_aa = 4
    if type(seqw) == int and seqw == 1: seqw = np.ones((1,N_seq))
    freq1, freq2, freq0 = freq(alg, Naa=4, seqw=seqw, lbda=lbda, freq0=freq0)
    # Overall fraction of gaps:
    theta = 1 - freq1.sum()/N_pos
    #theta = 0.0561639696805
...
def scaMat(alg, seqw=1, norm='frob',lbda=0, freq0=np.ones(4)/5,):
...
def icListForDC(Vpica, kpos, Csca, p_cut=0.95):
    ''' Produces a list of positions contributing to each independent component (IC) above
    a defined statistical cutoff (p_cut, the cutoff on the CDF of the t-distribution
    fit to the histogram of each IC).  Any position above the cutoff on more than one IC
    are assigned to one IC based on which group of positions to which it shows a higher
    ...
    for k in range(kpos):
        ...
        diff = abs(tail - p_cut);
        x_pos = diff.argmin()
        cutoff.append(x_dist[min(x_pos+tmp, len(x_dist)-1)])
    #select the positions with significant contributions to each IC
    ic_init = list()
    for k in range(kpos): ic_init.append([i for i in range(Npos) if Vpica[i,k]> cutoff[k]])
    #construct the sorted, non-redundant iclist
...
def directInfo(freq1, freq2, lbda=.0005, freq0=np.ones(4)/5, Naa=4):
    ...
    for i in range(Npos):
        for j in range(i+1,Npos):
            u,s,vt = np.linalg.svd(Cmat_dat[Naa*i:Naa*(i+1), Naa*j:Naa*(j+1)])
            Cfrob_dat[i,j] = np.sqrt(sum(s**2))
            DI[i,j] = dirInfoFromJ(i, j, Jmat, frq, Naa)
            DI[j,i] = DI[i,j]
    return DI, Cfrob_dat

def dirInfoFromJ(i, j, Jmat, frq, Naa=4, epsilon=1e-4):
...
def randomize(msa_num, Ntrials, seqw=1, norm='frob', lbda=0, Naa=4, kmax=6):
...
def randomizeAPC(msa_num, Ntrials, seqw=1, norm='frob', lbda=0, Naa=4, kmax=6):
    ''' Randomize the alignment while preserving the frequencies of amino acids at each
    position and compute the resulting spectrum of the SCA matrix.
    ...
    **Returns:**
    ...
    for t in range(Ntrials):
        ...
        total_avg = 0.0
        column_avgs = []
        for i in range(0, len(Csca)):
            avg = 0
            for j in range(0, len(Csca)):
                if i == j:
                    continue
                total_avg += Csca[i][j]
                avg += Csca[i][j]
            avg  /= len(Csca[i]) -1
            column_avgs.append(avg)
        total_avg = total_avg/(len(Csca[0])*(len(Csca[0])-1))
        APC = Csca
        for i in range(0, len(Csca)):
            for j in range(0, len(Csca)):
                APC[i][j] = APC[i][j] - column_avgs[i]*column_avgs[j]/total_avg
...
def figColors():
	...
	for s in np.arange(0,1,.05):
		for a in np.arange(0,1,.01):
			...
			plt.plot(s*np.cos(2*np.pi*a), s*np.sin(2*np.pi*a),'o', markersize=30,\
...
def figUnits(v1, v2, units, marker='o', dotsize=9, notinunits=1):
    ...
    **Keyword Arguments**
    ...
    if notinunits == 1:
        ...
        plt.plot(notinunits[0],notinunits[1],marker,  linewidths=.1, markersize=dotsize,\
...
##########################################################################################
#Randomize phylogenetic tree to determine statistically significant sectors
def rndPhylogeneticTree(aln, Ntrials):
